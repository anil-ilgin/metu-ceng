...
Boolean saveImage ( Image image , char * fileName )
{
    ...
    for(i=0; i<(image.height * image.width); i++) {
        fprintf(fp,"%3u ", image.pixels[i].red);
        fprintf(fp,"%3u ", image.pixels[i].green);
        fprintf(fp,"%3u", image.pixels[i].blue);
        if( (i+1)%5 != 0) {fprintf(fp, " "); }
        if( i == 0 ) {continue;}
        else if( ((i+1)%5) == 0 ) { fprintf(fp, "\n"); }
    }
    fclose(fp);
}
...
)
{
  ...
  int i;
  Point temp1, temp2;
  double sum = ((endAngleDegrees - startAngleDegrees) / (double)numberOfSegments) ;
  double angle = 90 + startAngleDegrees ;

  for(i = 0; i< numberOfSegments ; i++) {
    temp1.col = center.col - (int)(radius * cos(angle * PI / 180.0));
    temp1.row = center.row - (int)(radius * sin(angle * PI / 180.0));
    angle += sum;
    temp2.col = center.col - (int)(radius * cos(angle * PI / 180.0));
    temp2.row = center.row - (int)(radius * sin(angle * PI / 180.0));
    drawLine(image,temp1,temp2,color);
  }
  return TRUE;
...
Boolean drawRectangle ( Image image , Point topLeft , unsigned short int width , unsigned short int height , Color color )
{
  ...
  Point to1,to2,to3;
  to1.row = topLeft.row;
  to1.col = topLeft.col + width;
  to2.row = to1.row + height;
  to2.col = to1.col;
  to3.row = to2.row;
  to3.col = topLeft.col;
  drawLine(image, topLeft, to1, color);
  drawLine(image, to1, to2, color);
  drawLine(image, topLeft, to3, color);
  drawLine(image, to3, to2, color);
  return TRUE;
}
...
Boolean drawRegularConvexPolygon ( Image image , Point center , unsigned short int radius , unsigned short int numberOfSides , Color color )
{
  ...
  int i;
  Point temp1, temp2;
  double sum = (360.0 / numberOfSides) ;
  double angle = 90.0;
  for(i = 0; i<numberOfSides; i++) {
    temp1.col = center.col - (radius * cos(angle * PI / 180.0));
    temp1.row = center.row - (radius * sin(angle * PI / 180.0));
    angle += sum;
    temp2.col = center.col - (radius * cos(angle * PI / 180.0));
    temp2.row = center.row - (radius * sin(angle * PI / 180.0));
    drawLine(image,temp1,temp2,color);
  }
  return TRUE;
...
Boolean drawCharacter ( Image image , Point topLeft , char c , Color color )
{
  ...
  int i,j;
  int temp, rem, k, bin[8];
  Point tp = topLeft;
  k = 7;
  if ((int)c == 13 ) { return FALSE;}
  for(i=12; i>=0; i--) {
     temp = (int)characterRasters[ascii - 32][i];
     if (temp == 0) {
       tp.row = tp.row + 1;
       k = 7;
       continue;
